...
static inline void parse_var(const char *name, const char *value) {
  ...
  if (!strcmp(name, "touchscreen") || !strcmp(name, "controller_cursor") ||
      ...
      !strcmp(name, "portrait")  || !strcmp(name, "ortho")) {
...
int read_config(const char *file) {
  ...
  config.language = LANG_AUTO;   /* follow the Switch system language unless overridden */
...
int write_config(const char *file) {
  ...
  fprintf(f,
    "# LAYTON BROTHERS: MYSTERY ROOM -- Switch wrapper config\n"
    "# screen_width / screen_height : render size in px (-1 = auto, 1920x1080 landscape)\n"
    "# language : ignored -- this build ships English assets only (no other-language scenes)\n");
