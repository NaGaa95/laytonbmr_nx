 ...
 * config.language is either a forced id (1..5) or 0 = AUTO, resolved once from the
...
static void *getproperty_value(const char *key) {
  ...
  call_n++;
  return jni_make_string(which == 2 ? "256" : "48000");
}
...
static void *act_object(const FakeID *id, va_list va) {
  ...
  if (name_has(id->name, "VersionName")) return jni_make_string(LBMR_VERSION_NAME);
  if (name_has(id->name, "PackageName")) return jni_make_string(LBMR_PACKAGE);
...
#define APP_VERSION_NAME LBMR_VERSION_NAME  /* "1.1.3" */
#define APP_VERSION_CODE LBMR_VERSION_CODE  /* 22 -- Layton has NO OBB (classic split-file assets) */
